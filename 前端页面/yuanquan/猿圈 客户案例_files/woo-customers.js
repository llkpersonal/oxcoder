  ...
  Woo.Pagine.prototype = {
    ...
    */
    setPager : function (){
      ...
      if( pg.isLastSub() ){
        // 以下是配置普通翻页器的html 字符串
        strPager = ['<div class="col-md-12 woo-pbr woo-mpbr"><section id=""><ul class="pagination woo-dib">',
            cup==1 ? '' : '<li><a class="woo-pre" href="#" pdir="-1" >上一页</a></li>' ,
            cup==1 ? '<li class="woo-cur"><a href="javascript:;">1</a></li>' : '<li><a href="javascript:;">1</a></li>',
            cup-nn > 2 ? '<li class="woo-ell" >…</li>' : '',
            (function (){
              var tmps = '';
              for(var i=cup-nn; i<=cup+nn; i++){
                if( i==cup && i!=1 ){
                  tmps += '<li class="woo-cur"><a href="javascript:;">'+i+'</a></li>';
                }else if(i>1 && i<=tn){
    ...
    _placeEachUnit : function(masn,c,$d,$data,htmlp,indom,resize,f){
      ...
      $htmlp = $drawer.find(c.unit).not('.woo-f,.woo-wait').each(function (i,e){
        ...
        if( (i === 0 && f && !c.rightAlignFirstBlock || i === colc - 1 && f && c.rightAlignFirstBlock) ){
          ...
          // 要添加的节点外层字符串
          addf = '<div class="woo col-md-3 col-xs-12 woo-f co'+minI+' '+(colwf?'woo-spcol':'')+'" data-ht="'+ht+'" data-idx="'+masn.unitCount+'" style="position:absolute;overflow:hidden;top:'+minY+'px;left:'+left+'px;width:'+(masn.firstWidth-c.columnMargin)+'px;"></div>',
        ...
        strwrap += '<div class="logo-item col-xs-12 col-sm-6 col-md-3 hvr-buzz-out '+c.unit.substr(1)+' co'+minI+(colwf?' woo-spcol':'')+'" '+ (id?'data-id="'+id+'"':'')+' data-ht="'+ht+'" data-idx="'+masn.unitCount+'" style="top:'+minY+'px;left:'+left+'px;"></div>';
