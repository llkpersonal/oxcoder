...
;(function (){
	...
	*/
	...
	<span>这是后台返回<%=i%><%=list[i].cname%>$</span>\*/
	/*</div></div>\*/
	//####################################################################
	TEMPLATES = [
	          	'<% for (var i = 0; i < list.length; i ++) { %>\
         		<% var u = list[i],indx = $unit(u.cid,u),olnk = $outlnk(u); %>\
         	<%if(u.isshow!=2){%>\
         		<% if(!indx) continue; %>\
	          	<!-- /内容 -->\
	          	<div class="woo">\
                    <section id="middle">\
                        <div class="panel panel-default shadow-effect">\
                            <div class="col-xs-12 panel-header">\
                                <div class="pull-left">\
                                    <h4 title="<%=u.position%>" class="line-control"><%if(u.isshow==1){%><a href="battle-detail.action?reid=<%=u.reid%>"><%}%><%=u.position%><%if(u.isshow==1){%></a><%}%></h4>\
                                    <h4 title="<%=u.cname%>" class="line-control"><%if(u.isshow==1){%><a href="user-corp-detail.action?cid=<%=u.cid%>"><%}%><%=u.cname%><%if(u.isshow==1){%></a><%}%></h4>\
                                </div>\
                                <div class="pull-right client-info hidden-md">\
                                    <span class="percent text-danger"><img class="img-circle" style="width:60px;height:60px" src="<%=u.logo%>"></span>\
                                </div>\
                                <% if(u.priority!=10){ %><span class="hot-tag">推荐</span><%}%>\
                            </div>\
                                ...
                                <ul class="list-unstyled">\
                                    ...
                                    <li>\
                                        <ul class="companyTags">\
								          	<%for(var j=0;j<u.taglist.length;j++){%>\
												<li><%=u.taglist[j].cut(8)%></li>\
							     			<%}%>\
                                        </ul>\
                                    </li>\
                                    <li>挑战项目：</li>\
                                    <li>\
							        <ul>\
						         		<%for(var j=0;j<u.pnamelist.length;j++){%>\
	          								<li><a href="index-battle-project-detail.action?pid=<%=u.pidlist[j]%>"><%=u.pnamelist[j].cut(30)%></a></li>\
						         		<%}%>\
						            </ul>\
                                    </li>\
                                    <li>难度：\
	          							<% for (var j = 0; j < u.difficulty; j ++) { %><i class="fa fa-star"></i><%}%>\
                                    <li><i class="fa fa-calendar"></i><%=u.starttime%>~<%=u.deadline%></li>\
                                    <li><i class="fa fa-user"></i><%=u.acceptCount%>人已接受挑战</li>\
                                </ul>\
                            </div><!-- /.panel-body -->\
                            <div class="panel-footer align-center">\
	          					<%if(u.isshow==1){%><a href="user-choose-challenge.action?reid=<%=u.reid%>"><button class="btn btn-new1">接受挑战</button></a><% } %>\
	          					<%if(u.isshow!=1){%><button disabled class="btn btn-new1">接受挑战</button><% } %>\
                            </div><!-- /.panel-footer -->\
                        </div>\
                    </section>\
                </div><!-- /内容 -->\
...
/*TEMPLATES = [
         	...
         	<%if(u.isshow!=2){%>\
         		<% if(!indx) continue; %>\
         		<div class="woo">\
         	<% if(u.priority!=10){ %><span style="position:absolute;top:0;left:0" class="label label-warning">猿圈Juan推荐</span><%}%>\
    <div class="panel panel-default project j">\
        <div class="panel-body">\
            <div class="row">\
                <div class="col-xs-12">\
                    <div class="pull-left">\
                        <h4><a href="javascript:;"><%=u.cname%></a></h4>\
                        <h5 class="text-muted"><%=u.cdes.cut(14)%></h5>\
                    </div>\
            ...
            <hr>\
            <ul class="list-unstyled">\
                <li>职位 <span class="pull-right"><%=u.position%></span></li>\
                <li>月薪 <span class="pull-right"><%=u.salary%>元</span></li>\
                <li>公司所在地 <span class="pull-right"><%=u.city%></span></li>\
            </ul>\
         	<div><strong>职位简介</strong></div>\
         	<div class="well"><%=u.redesc%></div>\
         	...
         	<small class="text-muted pull-right text-default">发表于<%=u.time%></small>\
        </div><!-- /.panel-body -->\
        <div class="panel-footer">\
            <%if(u.isshow==1){%><a style="color:white" href="user-choose-challenge.action?reid=<%=u.reid%>" class="btn btn-success">接受挑战</a><% } %>\
         	<%if(u.isshow==2){%><button style="color:white" disabled class="btn btn-default">已经接受</button><% } %>\
		    <a style="color:white" href="battle-detail.action?reid=<%=u.reid%>" class="btn btn-info">详情></a>\
        </div><!-- /.panel-footer -->\
    </div><!-- /.panel -->\
</div>\<%}%><% } %>',
         	null,
			...
			<div class="woo">\
		        <div class="j panel \
		            ...
		            <div class="panel-heading">\
		                <h3 class="panel-title">这是一个很好很好的公司</h3>\
		            </div><!-- /.panel-heading -->\
		            <div class="panel-body">\
		                <p><%=u.desc%></p>\
		            </div><!-- /.panel-body -->\
		            <div class="panel-footer">\
		                <a href="#" class="btn btn-primary">接受挑战</a>\
